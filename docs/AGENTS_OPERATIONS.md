...
- **Nouveaux modes Google-only** : la commande `python -m app sync --mode google_pending` (ou dans l’UI) relance uniquement l’enrichissement Google pour les établissements jamais vérifiés, en envoyant les alertes correspondantes. Le mode `--mode google_refresh` purgera au préalable toutes les fiches Google stockées pour relancer la détection complète (sans déclencher d’e-mails). Le scheduler interne conserve la logique historique (`full`).
- **Rejeu ciblé d’une journée** : le paramètre `--mode day_replay --replay-for-date YYYY-MM-DD` (ou la modale front) rejoue intégralement une date précise. Les alertes ainsi générées sont restreintes aux administrateurs et les curseurs (`sync_state`) ne sont pas modifiés, ce qui permet d’investiguer sans perturber le run quotidien.
